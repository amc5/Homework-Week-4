...
h<- mean(d$height, na.rm = T)
h
hsd<- sqrt(sum(x-h)^2/length(x))
hsd
sdv(d$height) ##line 20 and 222 are the same opperation, one using the above function
w<- mean(d$weight, na.rm = T)
w
sdv(d$weight)
a<- mean(d$age, na.rm = T)
a
sdv(d$age)
k<- mean(d$zombies_killed, na.rm = T)
k
sdv(d$zombies_killed)
e<- mean(d$years_of_education, na.rm = T)
e
...
```{r}
ggplot(d, aes(x=age, y=height))+geom_point() ##I put age on the x axis because this makes it easier for me to compare the two different scatterplots
ggplot(d, aes(x=age, y=weight))+geom_point()
...
```{r}
hist(d$height, col="pink")
hist(d$weight, col="yellow")
hist(d$age, col="brown") ###don't be fooled, this is more of a burnt adobe color
hist(d$zombies_killed, col="turquoise")
hist(d$years_of_education, col="purple")

qqnorm(d$height, col="deeppink2")
qqnorm(d$weight, col="goldenrod1")
qqnorm(d$age, col="saddlebrown")
qqnorm(d$zombies_killed, col="mediumaquamarine")
qqnorm(d$years_of_education, col= "orchid4")
